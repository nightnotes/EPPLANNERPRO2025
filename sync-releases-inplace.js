// EP Planner — Releases sync (drop-in replacement for sync-releases-inplace.js)
// CSP-safe: no external scripts. Uses Firestore REST. Realtime-ish via 2s polling.
...
function encodeDocument(list){ return { fields: { value: toFirestoreValue(list) } }; }
...
// Boot
let saving=false, lastApplied=null;
...
(async()=>{
  // Wait for the table to exist (page render)
