...
fetch('/artists.json', { cache: 'no-store' }).then(r=>r.json()).then(list=>{ ARTISTS = list; renderList(); updateGrandTotal(); });
...
// ---- Persisted series from Netlify (overrides local data) ----
window.PERSIST_SERIES = [];
async function loadPersistentSeries(){
  try {
    const r = await fetch('/.netlify/functions/getData?key=streams_timeseries', { cache: 'no-store' });
    const data = await r.json();
    if (Array.isArray(data)) {
      window.PERSIST_SERIES = data;
    } else {
      window.PERSIST_SERIES = [];
    }
  } catch (e) { window.PERSIST_SERIES = []; }
}
async function savePersistentPoint(dateStr, streams){
  await fetch('/.netlify/functions/appendStreams', {
    method: 'POST',
    headers: {'content-type':'application/json'},
    body: JSON.stringify({ date: dateStr, streams: Number(streams)||0 })
  });
  await loadPersistentSeries();
  renderChart();
}
